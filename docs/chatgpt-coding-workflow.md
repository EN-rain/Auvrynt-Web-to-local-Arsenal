...
Auvrynt brings a Codex-style coding-agent loop to ChatGPT and other MCP hosts:
...
Checkout mode is the default. Auvrynt opens the actual directory:
...
Auvrynt reports when the source checkout was dirty so the model can decide how
to proceed with the user.

## Project Instructions

When a workspace opens, Auvrynt loads root-level instruction files:
...
Auvrynt discovers skills from:

- `AUVRYNT_AGENT_DIR`, which defaults to `~/.codex`
- project `.pi/skills`
- optional paths from `AUVRYNT_SKILL_PATHS`

When `open_workspace` returns matching skills, the model should read the
advertised `SKILL.md` before following that skill.

Skill paths may be outside the workspace. Auvrynt only permits reading:

- advertised `SKILL.md` files
- files under a skill directory after that skill's `SKILL.md` has been read

Set `AUVRYNT_SKILLS=0` to hide skills from workspace output.
...
By default, Auvrynt also runs in `AUVRYNT_TOOL_MODE=minimal`, so dedicated
...
Legacy names are available with `AUVRYNT_TOOL_NAMING=legacy`:
...
Use `AUVRYNT_TOOL_MODE=full` to restore dedicated search and directory tools.

## Show Changes

By default, `AUVRYNT_WIDGETS=full`.

In that mode, Auvrynt attaches widget UI to the exposed workspace, file, edit,
and shell tools. The aggregate `show_changes` tool is not exposed by default.

Use `AUVRYNT_WIDGETS=off` to disable widget UI, or `AUVRYNT_WIDGETS=changes`
