...
export function parseDotnetBuildOutput(output: string): {
  ...
  // Example MSBuild diagnostic line:
  // C:\project\Program.cs(12,15): error CS1002: ; expected [C:\project\project.csproj]
...
export function parseDotnetTestOutput(output: string): {
  ...
  // Summary line: Passed! - Failed: 1, Passed: 5, Skipped: 0, Total: 6
  ...
  for (const line of lines) {
    if (line.includes("Failed ") || line.includes("Failed:")) {
      ...
      if (nameMatch && !failedTestNames.includes(nameMatch[1])) {
        failedTestNames.push(nameMatch[1]);
      }
    }
...
export async function inspectDotnetProject(
  ...
  let pkgMatch;
  while ((pkgMatch = pkgRegex.exec(xmlContent)) !== null) {
    packageReferences.push(pkgMatch[1]);
  }

  const projectReferences: string[] = [];
  const projRefRegex = /<ProjectReference\s+Include="([^"]+)"/g;
  let projRefMatch;
  while ((projRefMatch = projRefRegex.exec(xmlContent)) !== null) {
    projectReferences.push(projRefMatch[1]);
  }
...
export async function dotnetBuild(
  ...
  try {
    ...
    const parsed = parseDotnetBuildOutput(fullOutput);
    return {
...
export async function dotnetTest(
  ...
  try {
    const { stdout, stderr } = await execFileAsync("dotnet", args, { cwd });
    const parsed = parseDotnetTestOutput(stdout + "\n" + stderr);
    return {
      ...parsed,
      durationMs: Date.now() - start,
    };
  } catch (err: any) {
    const parsed = parseDotnetTestOutput((err.stdout ?? "") + "\n" + (err.stderr ?? err.message));
    return {
      ...parsed,
      durationMs: Date.now() - start,
    };
