...
// Bridge-based animation listing (requires live editor bridge)
export async function listAnimations(
  ...
  const result = await bridgeClient.sendRequest("animation.list", {
...
export async function createAnimation(
  ...
  await bridgeClient.sendRequest("animation.create", {
    nodePath: input.animationPlayerNodePath,
    animationName: input.animationName,
    length: input.lengthSeconds ?? 1.0,
...
export async function validateAnimation(
  ...
  if (!bridgeClient.status) throw new Error("Godot Editor Bridge is not connected.");
  const anim = await getAnimation(input, bridgeClient);
  const issues: string[] = [];

  if (anim.lengthSeconds === 0) issues.push("Animation length is zero.");
...
export async function validateTilemap(
  ...
  if (!bridgeClient.status) throw new Error("Godot Editor Bridge is not connected.");
  const result = await bridgeClient.sendRequest("tilemap.validate", { nodePath: input.tileMapNodePath });
  return result;
}
...
export async function validateCollisions(
  ...
  if (!bridgeClient.status) throw new Error("Godot Editor Bridge is not connected.");
  const result = await bridgeClient.sendRequest("physics.validate_collisions", { nodePath: input.nodePath ?? "/" });
  return result;
}
