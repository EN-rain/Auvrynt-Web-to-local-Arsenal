...
export async function inspectGodotDotnetProject(
  ...
  async function scanDir(dir: string) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (entry.name === ".godot" || entry.name === "bin" || entry.name === "obj" || entry.name === "node_modules" || entry.name === ".git") {
          continue;
        }
  ...
  if (csprojFile) {
    try {
      ...
      let m;
      while ((m = pkgRegex.exec(xml)) !== null) packageReferences.push(m[1]);
    } catch {}
  }
