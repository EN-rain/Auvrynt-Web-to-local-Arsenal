...
export async function godotRunProject(
  ...
  if (input.userArguments && input.userArguments.length > 0) {
    args.push("--", ...input.userArguments);
  }

  const command = `${godotExe} ${args.join(" ")}`;
...
export function parseStructuredRuntimeLogs(rawLines: string[]): StructuredRuntimeLogEntry[] {
  ...
  for (const line of rawLines) {
    ...
    // Stack frame line: at Namespace.Class.Method(Type arg) in C:\path\File.cs:line 45
  ...
  // Group repeated identical messages
  const grouped: StructuredRuntimeLogEntry[] = [];
  for (const entry of entries) {
    const prev = grouped[grouped.length - 1];
    if (prev && prev.message === entry.message && prev.severity === entry.severity) {
...
export function getGodotRuntimeLogs(
  ...
  const filtered = severityFilter
    ? parsed.filter((e) => severityFilter.has(e.severity))
    : parsed;
