...
const root = await mkdtemp(join(tmpdir(), "auvrynt-review-checkpoints-test-"));

try {
  await git(root, ["init"]);
  await git(root, ["config", "user.email", "auvrynt@example.com"]);
  await git(root, ["config", "user.name", "Auvrynt Test"]);
