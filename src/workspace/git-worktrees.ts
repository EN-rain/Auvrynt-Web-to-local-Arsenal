...
const MAX_MANAGED_WORKTREES = 16;
...
export async function createManagedWorktree(input: {
  ...
  const existing = await readdir(input.config.worktreeRoot).catch(() => [] as string[]);
  const managedCount = existing.filter((e) => /^[a-zA-Z0-9._-]+-[a-f0-9]{8}$/.test(e)).length;
  if (managedCount >= MAX_MANAGED_WORKTREES) {
    throw new GitWorktreeError(
      "GIT_WORKTREE_CREATE_FAILED",
      `Cannot create managed worktree: limit of ${MAX_MANAGED_WORKTREES} concurrent managed worktrees reached. Remove unused worktrees and retry.`,
    );
  }
