...
const REVIEW_REF_PREFIX = "refs/auvrynt/review";
...
async function createWorkingTreeSnapshot(gitRoot: string): Promise<string> {
  const tempDir = await mkdtemp(join(tmpdir(), "auvrynt-review-index-"));
  ...
  try {
    ...
    return (await git(gitRoot, ["commit-tree", tree, "-p", parent, "-m", "Auvrynt review snapshot"], { env })).stdout.trim();
  } finally {
...
function checkpointEnv(indexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "Auvrynt",
    GIT_AUTHOR_EMAIL: "auvrynt@users.noreply.local",
    GIT_COMMITTER_NAME: "Auvrynt",
    GIT_COMMITTER_EMAIL: "auvrynt@users.noreply.local",
  };
}
